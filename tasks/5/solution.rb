class HashStore
  ...
  def create(input_hash)
    @storage[input_hash[:id]] = input_hash
  end

  def find(input_hash)
    @storage.values.select do |record|
      input_hash.all? { |key, value| record[key] == value }
    end
  end

  def delete(input_hash)
    find(input_hash).each { |element| @storage.delete(element[:id]) }
  end

  def update(id, input_hash)
    @storage[id].merge!(input_hash)
  end
end
...
class ArrayStore
  ...
  def find(input_hash)
    @storage.select { |record| match_record? input_hash, record }
  end

  def delete(input_hash)
    @storage.reject! { |record| match_record? input_hash, record }
  end
  ...
  private
  def match_record?(input_hash, record)
    input_hash.all? { |key, value| record[key] == value }
  end
end
